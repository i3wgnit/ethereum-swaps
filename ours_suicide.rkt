...
(seq sub_0
     (;; ===
      ...
      (dest fun:expire)
      (jumpi tag_34 (gt (timestamp) (sload s_expiration)))
      (jump #x2)
      (jump tag_38)
      ...
      (dest fun:claim)
      (jumpi tag_91 (iszero (iszero (eq (caller) (sload s_recipient)))))
      (invalid)

      (dest tag_91)
      (mstore 0 (calldataload 4))
      (mstore 0 (keccak256 0 32))
      (jumpi tag_114 (eq (sload s_keyHash) (mload 0)))
      (jump #x2)
      (jump tag_118)

      (dest tag_114)
      (selfdestruct (sload s_recipient))
      (stop)
